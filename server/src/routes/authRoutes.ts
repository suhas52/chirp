...
authRouter.post("/register", async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!inputValidation.success) {
        return next(new CustomError("Failed to validate input", 400))
    }
...
authRouter.post("/login", async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!inputValidation.success) return next(new CustomError("Invalid input", 400))
...
authRouter.patch("/update-avatar", upload.single('avatar'), async (req: Request, res: Response, next: NextFunction) => {
    const image = req.file;
    if (!req.cookies.token) return next(new CustomError("User not logged in", 401))
    const accessToken = req.cookies.token;
    const decodedUser = jwt.verify(accessToken, SECRET) as DecodedUser;
    if (!image) return next(new CustomError("Image not provided", 400))
    if (!ALLOWED_FILE_TYPES.includes(image.mimetype)) return next(new CustomError("Invalid file type", 400))
